...
export const CategoryList = () => {
    ...
    const filteredCategories = categories.filter(cat =>
        ...
        (cat.productGroup?.name && cat.productGroup.name.toLowerCase().includes(searchTerm.toLowerCase()))
    );
            ...
            <div className="bg-white rounded-[32px] flex-1 flex flex-col shadow-sm border border-slate-100 overflow-hidden">
                ...
                <div className="px-8 py-6 bg-[#dbeafe]/50 border-b border-slate-100 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-slate-500 text-sm mt-1">Organize your products into logical categories</p>
                    </div>
                    <button
                        onClick={() => navigate('/settings/masters/category/new')}
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="text-slate-400 text-sm font-semibold uppercase tracking-wider">
                                    ...
                                    <th className="pb-4 px-4">Product Group</th>
                                    <th className="pb-4 px-4">Tax</th>
                                    ...
                                    <tr key={cat.id} className="hover:bg-slate-50/50 transition-colors group">
                                        ...
                                        <td className="py-4 px-4">
                                            <span className="px-3 py-1 bg-slate-100 text-slate-600 rounded-lg text-xs font-semibold">
                                                {cat.productGroup?.name || 'Unassigned'}
                                            </span>
                                        </td>
                                        <td className="py-4 px-4 text-slate-600 font-medium">{cat.tax || '0%'}</td>
                                        <td className="py-4 px-4">
                                            <span className={`px-3 py-1 rounded-lg text-xs font-bold ${cat.status === 'Active'
                                        ...
                                        <td className="py-4 px-4 text-right">
                                            <div className="flex justify-end gap-2 ">
                                                <button
                                                    onClick={() => navigate(`/settings/masters/category/${cat.id}`)}
