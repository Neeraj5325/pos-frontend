...
import { Plus, Edit2, Trash2, Search, User, MoreVertical, Smartphone, Mail } from 'lucide-react';
        ...
        <div className="flex-1 flex flex-col h-full bg-[#f3f3f3] overflow-hidden">
            <div className="bg-white rounded-[32px] flex-1 flex flex-col shadow-sm border border-slate-100 overflow-hidden">
                ...
                <div className="px-8 py-6 bg-[#dbeafe]/50 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <button
                        onClick={() => navigate('/settings/user-security/users/new')}
                            ...
                            <tbody className="divide-y divide-slate-100 text-sm">
                                {filteredUsers.map((user) => (
                                    <tr key={user.id} className="hover:bg-slate-50/50 transition-colors group">
                                        ...
                                        <td className="py-4 px-4">
                                            <span className="px-3 py-1 bg-blue-50 text-blue-600 rounded-lg text-xs font-semibold uppercase">
                                                {user.role}
                                            </span>
                                        </td>
                                        ...
                                        <td className="py-4 px-4">
                                            <span className={`px-3 py-1 rounded-lg text-xs font-semibold ${user.status === 'Active' ? 'bg-green-50 text-green-600' : 'bg-slate-100 text-slate-500'
                                                }`}>
                                                {user.status || 'Active'}
                                            </span>
                                        </td>
                                        <td className="py-4 px-4 text-right">
                                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button
                                                    onClick={() => navigate(`/settings/user-security/users/${user.id}`)}
